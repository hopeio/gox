...
// Set adds a Node with the given handle to the path.
// Not concurrency-safe!
func (n *Node[T]) Set(path string, data T) {
	fullPath := path
	n.priority++
...
// given path.
func (n *Node[T]) Get(path string) (data T, params Params, tsr *Node[T]) {
		...
		if len(path) > len(prefix) {
			...
			for i, c := range n.indices {
				if c == '/' {
					...
					if n.nType == catchAll && n.children[0].ok {
						tsr = n.children[0]
					}
					return
				}
...
// was successful.
func (n *Node[T]) findCaseInsensitivePath(path, method string, fixTrailingSlash bool) (fixedPath string, found bool) {
...
func (n *Node[T]) findCaseInsensitivePathRec(path, method string, ciPath []byte, rb [4]byte, fixTrailingSlash bool) []byte {
	npLen := len(n.path)
